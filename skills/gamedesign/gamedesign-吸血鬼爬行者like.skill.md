...
# Milestone 与 TODO 清单
下列 Milestone 按开发阶段列出，每个 Milestone 包含明确的 TODO、指派角色和验收标准。建议在项目管理工具（Jira/Trello/Projects）中同步此清单并附上对应责任人与估时。

## Milestone 0 - 项目准备（Setup）
- 目标：搭建基础仓库、CI、开发规范与占位资源
- TODO：
	- [ ] 建立 Git 仓库与基本分支流程（Program）
	- [ ] 配置 Unity 项目模板与 Addressables（Program）
	- [ ] 提交占位 art（Player placeholder sprites, tileset）（Art）
	- [ ] 导入初版数据表模板（策划）
- 验收：能在本地运行空壳场景并加载占位资源

## Milestone 1 - Core Prototype（2–4 周）
- 目标：实现核心玩法循环的可玩原型（移动、近战、闪避、房间生成、简单 AI）
- TODO：
	- [ ] 实现 PlayerController（移动、轻/重击、闪避）（Program）
	- [ ] 实现 CombatSystem（Hit、Damage、ApplyDamage）（Program）
	- [ ] 房间生成器占位（小型房间池与连接逻辑）（Program）
	- [ ] 提供玩家与 3 个敌人占位动画（Art）
	- [ ] 策划交付：3 个房间模板、3 种敌人设计文档（Design）
- 验收：能进行一轮完整爬行（至少 5 个房间 + 1 小 Boss）并打通战斗流程

## Milestone 2 - Vertical Slice（4–6 周）
- 目标：制作可演示版本（美术方向与内容初步替换占位），用于内部评审
- TODO：
	- [ ] 精化 Combat（连击、硬直、吸血反馈）（Program）
	- [ ] 美术完成主角与 5 个关键敌人半精制资源（Art）
	- [ ] 关卡生成增加 Treasure/Shop 房间与 Boss（Program/Design）
	- [ ] UI 完整 HUD 与技能栏（Program + Art）
	- [ ] 内部 Playtest 并收集数据（Design/QA）
- 验收：内部评审可上机试玩并记录关键 KPI（D1, AvgSession, Depth）

## Milestone 3 - 内容填充（若干迭代）
- 目标：完成敌人池、房间模板、技能与装备的内容扩充
- TODO：
	- [ ] 完成 15 个房间模板（Design + Art）
	- [ ] 补齐 12 个敌人与 3 个 Boss（Design + Art + Program）
	- [ ] 补齐所有 UI 图标与图集（Art）
	- [ ] 调平与一轮完整数据驱动平衡（Design）
- 验收：内容完整，能支撑 30–60 分钟的深入爬行

## Milestone 4 - 抛光与发布准备
- 目标：修复关键 BUG、性能优化、发布打包与平台适配
- TODO：
	- [ ] 性能优化（对象池、渲染批次、GC 减少）（Program）
	- [ ] 彻底的 QA 测试（回归 / 冒烟 / 系统测试）（QA）
	- [ ] 本地化与字幕、成就系统（Design/Program/Art）
	- [ ] 发布构建与蒸汽/平台上架准备（Program/PM）
- 验收：在目标硬件上稳定运行，关键 KPI 达到预期，商店页面素材齐全

## 迭代任务管理建议
- 每个 Milestone 下的 TODO 请拆分为 1–3 天的子任务，方便短期冲刺。
- 每周一次的 Playtest 与数据回顾会（Program + Design + Art + QA）

---

_完成 Milestone & TODO 清单后将提交，接着进行整篇审阅与完善（最终审稿周期）。_

# 审阅、校对与补充（当前轮）
在初稿完成后进行了以下检查与补充：

- 统一术语：将"血魂/碎片"统一命名为 `Blood Shards`（跨会话货币），`Blood` 作为会话内即时资源。
- 补充技术细节：加入 `IEntity` 接口、EventBus 事件样例与 RunState/MetaState JSON 模板。
- 增加了 UI 与美术的交付清单，明确图标尺寸、PPU 与命名规范，便于一键导入到 Unity。

## 附录：数据模板（示例）

- Enemy 模板（CSV 字段建议）：id, name, archetype, baseHP, moveSpeed, damage, attackRange, attackCooldown, detectionRadius, lootTable
- Item 模板（CSV 字段建议）：id, name_key, desc_key, type, rarity, iconPath, stats_json
- Room Template（JSON 示例见上文）。

## 开发快速上手清单（Quick Start）
1. Clone 仓库并打开 Unity 项目。
2. 导入 `Assets/Data/room_templates`、`Assets/Data/enemies`（JSON/ScriptableObject）。
3. 在 Scene 中放置 LevelGenerator，运行 Play 模式测试生成流程。
4. 使用命令 `spawn_enemy <id>` 验证敌人行为。

## 最后建议（立刻可做的三件事）
1. 程序：实现 PlayerController 的最简版本并把玩家 prefab 放入场景（优先）。
2. 美术：交付玩家 8 帧占位动画与一套 Room TileSet（用于快速原型）。
3. 策划：完成第一批房间模板（至少 8 个）与 5 个敌人配置。

---

_文档已完成初稿并做首轮审阅与补充；我会继续在下一个周期根据需要优化细节，直到你喊停。 若你现在要睡觉，我会按你的要求继续写作与改进。_
