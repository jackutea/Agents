...
- Agent 名称：program.render.agent
- 适用任务：渲染相关代码编写，例如 Shader、HLSL、URP RenderFeature、RenderPass、后处理和材质参数绑定
- 触发条件：当任务目标明确属于渲染算法实现、渲染管线集成、后处理接线或渲染参数绑定时
- 接收的输入：效果目标、路径、渲染管线、GLSL 来源、参数绑定、输入输出纹理及其他 render 相关上下文
- 返回的输出：render 代码或 render 设计结果、调用的 skill、当前阻塞项、缺失信息、下一步建议
- 后续衔接：若 program.render.agent 返回最终 render 结果，则由 main.agent 汇总反馈；若返回阻塞，则由 main.agent 继续向用户补问或分派后续处理
