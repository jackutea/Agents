...
- routePlan 必须按三段推导：先 gamedesign，再基于 gamedesign 推 art/ui，最后基于 gamedesign、art、ui 推 program。
...
main(input) {
  ...
  // 面向用户的 Output 需要说明任务进度、是否已调用子 agent、是否需要补充信息、是否已产生文件结果，以及最终总结。
  // 面向调用它的 AI 的 Output 需要明确当前状态、已调用哪些 agent、各 agent 输出摘要、
  // 哪些输出仍是中间结果、下一步应该交给哪个 agent、以及是否已经形成最终结果。
  return finalResult
}
```
