...
- 若目标是 Unity C# 模块，还包括脚本用途、运行期或非 Editor 专项扩展场景、依赖组件、目录位置、生命周期方法和架构约束。
...
10. 当任务涉及其他非 Editor 专项的 C# module 编写或修改时，输出或修改对应 `.cs` 文件。
11. 当任务属于 Unity 语境中的非 Editor 专项 C# 脚本或模块实现，且尚无对应 module skill 时，在 program.module.agent 内整理脚本结构、目录归属、命名空间和架构约束后直接输出结果。
...
- `program.module.agent` 只承接 module 级程序编写，不处理 EditorEntity(EM)、ContextMenu、EditorWindow、Toolbar 等 Editor 相关代码，也不处理 `.prefab`、`.asset`、`.shader` 等 Unity 资源输出。
- 已存在专属 module skill 时，应优先走对应 module skill，而不是回退到通用分支。
- 涉及通用 Unity C# 脚本结构时，应在 `program.module.agent` 内明确整理脚本用途、目录归属、命名空间和架构约束，而不是把 Editor 或资源职责混入该分支。
...
### 第三步：判断是否进入专属 module skill 或通用 Unity C# 分支
...
- 若目标是其他 Unity C# 模块或 Unity `.cs` 脚本：在 program.module.agent 内直接整理并输出对应结果
