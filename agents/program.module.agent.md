...
7. 当任务涉及其他 C# module 编写或修改时，输出或修改对应 `.cs` 文件。
8. 当任务属于 Unity 语境中的 C# 脚本或模块实现，且尚无对应 module skill 时，调用 `unity-csharp.skill.md` 处理具体脚本结构与架构约束。
9. 当任务仅是普通 module 结构调整时，直接给出目标模块的文件结果或修改建议。
10. 若信息不足，先向调用方返回缺失项和下一步建议。
