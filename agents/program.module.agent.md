...
当任务仍属于 Unity 语境中的 `.cs` 文件编写时，program.module.agent 负责承接上游分派，再按需要调用 `unity-csharp.skill.md` 完成具体脚本结构与架构约束处理；它不负责 EditorEntity(EM)、ContextMenu、EditorWindow、Toolbar 等 Editor 相关代码，也不负责 Unity 资源文件本身的生成，不替代专门的风格审查或性能分析 agent。
...
- 若目标是 Unity C# 模块，还包括脚本用途、运行期或非 Editor 专项扩展场景、依赖组件、目录位置和架构约束。
...
11. 当任务属于 Unity 语境中的非 Editor 专项 C# 脚本或模块实现，且尚无对应 module skill 时，调用 `unity-csharp.skill.md` 处理具体脚本结构与架构约束。
