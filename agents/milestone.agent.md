...
- 在用户工程根目录下优先读取 /AI-User/docs/Milestone.md；若不存在则创建，并在后续交互中持续增量更新。
...
milestone(input) {
	...
	var milestoneDoc = readOrCreateMilestoneDoc(input, "/AI-User/docs/Milestone.md")
...
- 必须在用户工程目录下优先读取 /AI-User/docs/Milestone.md；若不存在则创建，并在每次拆解后增量更新。
