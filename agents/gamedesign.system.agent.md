...
gamedesignSystemAgent(input) {
  ...
  if (isQuestSystem(systemType, input)) {
    // Output: 返回任务系统设计结果。
    return gamedesign-system-quest.skill(input)
  }

  if (isDialogueSystem(systemType, input)) {
    // Output: 返回对话系统设计结果。
    return gamedesign-system-dialogue.skill(input)
  }
  ...
  return finalizeSystemDesign(systemLoop, rules, progression, feedback, risks)
}
```
