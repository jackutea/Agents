---
name: unity
description: "处理 Unity 工程初始化与基础文本资源文件生成，包括根据 Unity 版本创建 .gitignore、根据 /gists/.editorconfig.gist.md 创建 .editorconfig；当任务涉及 Unity 策划内容时，转派给 unity.gamedesign.agent；当任务涉及 Unity 内部美术内容时，转派给 unity.art.agent；当任务涉及 Unity C# 编程时，转派给 program.module.agent。"
model: GPT-5.4
...
unity.agent 负责 Unity 项目初始化与基础文本资源文件生成。

它负责把 Unity 初始化和基础文本资源侧需求分派到合适的 skill，或在明确约束下直接生成 Unity 文本资源文件；当任务涉及 ScriptableObject 等 Unity 策划内容时，它不再直接承接，而是转派给 `unity.gamedesign.agent.md`；当任务涉及 animation、animator、render、prefab 等 Unity 内部美术内容时，它转派给 `unity.art.agent.md`；当任务涉及 Unity C# 代码编写时，它转派给 `program.module.agent.md`；它不替代与 Unity 无关的通用 agent。

## 接收的 Input

unity.agent 接收以下 Input：

- 用户或调用方提出的 Unity 工程初始化需求、基础文本资源创建需求，或需要 Unity 侧进一步转派的策划 / 美术 / C# 编程需求。
- 当前 Unity 项目的目录结构、资源路径、命名约定、目标文件位置。
- Unity 版本、资源类型、目标平台、是否已有对应资源。
- 若任务最终会转派到 unity.gamedesign.agent，还包括 ScriptableObject 类型名、资源名称、输出路径、GUID 上下文等策划侧信息。
...
4. 当任务涉及 ScriptableObject 等 Unity 策划内容时，转派给 `unity.gamedesign.agent.md`。
...
8. 在执行前，检查路径、命名、Unity 版本、资源类型，以及转派所需的策划 / 美术 / 脚本上下文是否齐全。
...
unity.agent 的任务编排是先区分初始化与基础文本资源任务、策划任务、美术任务、C# 编程任务；初始化与基础文本资源任务仍由自身处理，策划任务转派给 `unity.gamedesign.agent.md`，美术任务转派给 `unity.art.agent.md`，C# 编程任务转派给 `program.module.agent.md`。
...
unity(input) {
	...
	if (includesScriptableObject(taskTypes)) {
		results.push(unity.gamedesign.agent(input))
	}
...
- `unity.agent` 以 Unity 初始化与基础文本资源任务为主。
- `unity.agent` 可以把 Unity 策划需求转派给 `unity.gamedesign.agent.md`。
...
确认 Unity 版本、资源路径、命名规则、目标文件、脚本用途、架构约束，以及策划侧 / 美术侧转派所需信息是否齐全。
...
- ScriptableObject：`unity.gamedesign.agent.md`
...
- ScriptableObject 任务应先转派给 `unity.gamedesign.agent.md`。
...
- 能创建 `.editorconfig`
- 能在涉及 Unity 策划内容时正确转派到 `unity.gamedesign.agent.md`
