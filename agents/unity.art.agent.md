...
- 用户或调用方提出的 animation、animator、prefab 相关需求。
...
1. 识别当前任务属于 animation、animator、prefab 中的哪一种或哪几种。
...
4. 当任务涉及 UI prefab、节点层级、脚本挂载或 Addressables 标记时，调用 `unity-prefab.skill.md`。
5. 在执行前，检查资源路径、命名规则、GUID 上下文和动画状态等前置条件是否齐全。
...
unity.art.agent 的任务编排是先确认美术任务类型，再按 animation、animator、prefab 分派到对应 skill，最后汇总结果。
...
识别当前请求属于 animation、animator、prefab 中的哪一种或哪几种。

### 第二步：检查前置条件

确认资源路径、命名规则、关键帧需求、状态集合、GUID 上下文和 Addressables 要求是否齐全。
...
- 能编排 `unity-animation.skill.md`、`unity-animator.skill.md`、`unity-prefab.skill.md`
