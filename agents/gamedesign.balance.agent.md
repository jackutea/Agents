...
gamedesignBalanceAgent(input) {
  ...
  if (isEconomyBalanceTask(balanceType, input)) {
    // Output: 返回经济数值设计结果。
    return gamedesign-balance-economy.skill(input)
  }

  if (isCombatBalanceTask(balanceType, input)) {
    // Output: 返回战斗数值设计结果。
    return gamedesign-balance-combat.skill(input)
  }

  if (isGeneralBalanceTask(balanceType, input)) {
    // Output: 返回通用数值平衡设计结果。
    return gamedesign-balance.skill(input)
  }
  ...
  return finalizeBalanceDesign(progressionCurve, resourceLoop, rewardRules, difficultyPacing, risks)
}
```
