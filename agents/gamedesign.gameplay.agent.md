...
gamedesignGameplayAgent(input) {
  ...
  if (is2DPlatformerGameplay(gameplayType, input)) {
    // Output: 返回 2D 平台玩法设计结果。
    return gamedesign-gameplay-2dplatformer.skill(input)
  }

  if (is3DFpsGameplay(gameplayType, input)) {
    // Output: 返回 3D FPS 玩法设计结果。
    return gamedesign-gameplay-3dfps.skill(input)
  }
  ...
  return finalizeGameplayDesign(coreLoop, rules, feedback, risks)
}
```
